<template>
  <div class="container">
    <div class="columns">
      <div class="column">
        <h1 class="title"><span class="is-hidden-tablet">3: </span>{{ $t('message.yword_title') }}</h1>
      </div>
    </div>
    ...
    <div v-else>
      <div class="recContainer" v-if="!ysoundRecording">
        <div class="columns">
          <div class="column">
            <p>{{ $t('message.yword_instructions') }}</p>
          </div>
        </div>
        <vue-dictaphone class="is-mobile columns" @stop="handleRecording($event)" v-slot="{ isRecording, startRecording, stopRecording }">
          <div class="column is-1-desktop is-one-third-mobile has-text-centered record_button"  v-if="!isRecording" @click="startRecording">
            ...
            <p class="label is-size-4 has-text-centered">{{ $t('message.record')}}</p>
          </div>
          <div v-else class="column is-1-desktop is-one-third-mobile has-text-centered record_button is-recording"  @click="stopRecording">
            <ion-icon size="xxlarge" name="radio-button-on-outline" ></ion-icon>
            <p class="label is-size-4 has-text-centered">{{ $t('message.stop')}}</p>
          </div>
    ...
    <navigation :current="2" :deleteWarning="deleteExisting" :allowForward="madeRecording" />
  </div>
</template>
...
export default {
  ...
  mounted () {
    this.axios.get('/entries/' + this.$store.state.identifier)
      .then((response) => {
        if (response.data.data.attributes.yword) {
          this.ysoundRecording = response.data.data.attributes.yword
  ...
  methods : {
    ...
    postToApi (deletion_key) {
      ...
      if (this.blob) {
        formData.append('yword', this.blob);
      } else if (deletion_key) {
        formData.append('yword', '_destroy')
      }
